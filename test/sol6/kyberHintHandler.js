...
const MASK_OUT = 1;
const SPLIT = 2;
const BPS_SPLIT = ['3000', '1000', '1000', '5000'];
const TRADE_TYPES = {
    MASK_IN,
    MASK_OUT,
    SPLIT,
...
const INVALID_SPLIT_BPS = {
    MORE_THAN_10000BPS: {value: ['2000', '3000', '5000', '6000'], revertMsg: 'total BPS != 10000'}, // more than 10000bps
    LESS_THAN_10000BPS: {value: ['1000', '500', '3000', '4000'], revertMsg: 'total BPS != 10000'}, // less than 10000bos
...
contract('KyberHintHandler', function(accounts) {
    ...
    describe("test building various hints", function() {
        describe("Token to ETH (T2E)", function() {
            before('one time init of vars', async() => {
                t2eReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xaa7777cc1234500f'];
                t2eDupReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xff1234567a334f7d'];
                t2eMissingReserves = ['0xff1234567a334f7d', '0xbb12aa56bbfff000', '0xcc12345675fff057', '0xaa7777cc1234500f']; // 0xbb12aa56bbfff000 doesn't exist
            });
        ...
        describe("ETH to Token (E2T)", function() {
            before('one time init of vars', async() => {
                e2tReserves = ['0xff12345663820d8f', '0xaa12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057'];
                e2tDupReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xff1234567a334f7d'];
                e2tMissingReserves = ['0xff12345663820d8f', '0xbb12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057']; // 0xbb12345616709a5d doesn't exist
            });
    ...
    describe("test parsing various hints", function() {
        describe("Token to ETH (T2E)", function() {
            before('one time init of vars', async() => {
                t2eReserves = ['0xff12345663820d8f', '0xaa12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057'];
            });
        ...
        describe("ETH to Token (E2T)", function() {
            before('one time init of vars', async() => {
                e2tReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xaa7777cc1234500f'];
            });
    ...
    describe("test parsing various incorrect hints", function() {
        describe("Token to ETH (T2E)", function() {
            before('one time init of vars', async() => {
                t2eReserves = ['0xff12345663820d8f', '0xaa12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057'];                
                t2eDupReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xff1234567a334f7d'];
                t2eMissingReserves = ['0xff12345663820d8f', '0xbb12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057']; // 0xbb12345616709a5d doesn't exist
                t2eDescendingReserves = ['0xff12345663820d8f', '0xcc12345675fff057', '0xaa12aa56bbfff000', '0xaa12345616709a5d'];                
            });
            ...
            it('should revert for T2E hint for SPLIT due to reserveIds NOT IN INCREASING ORDER', async() => {
                ...
                hint = buildHint()(t2eOpcode, t2eDescendingReserves, t2eSplits);
                await expectRevert(
                    hintHandler.parseTokenToEthHint(hint),
                    'reserveIds not in increasing order'
                );
        ...
        describe("ETH to Token (E2T)", function() {
            before('one time init of vars', async() => {
                e2tReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xaa7777cc1234500f'];
                e2tDupReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xff1234567a334f7d'];
                e2tMissingReserves = ['0xff1234567a334f7d', '0xbb12aa56bbfff000', '0xcc12345675fff057', '0xaa7777cc1234500f']; // 0xbb12aa56bbfff000 doesn't exist
                e2tDescendingReserves = ['0xff1234567a334f7d', '0xcc12345675fff057', '0xaa7777cc1234500f', '0xaa12aa56bbfff000'];
            });
            ...
            it('should revert for E2T hint for SPLIT due to reserveIds NOT IN INCREASING ORDER', async() => {
                ...
                hint = buildHint()(e2tOpcode, e2tDescendingReserves, e2tSplits);
                await expectRevert(
                    hintHandler.parseEthToTokenHint(hint),
                    'reserveIds not in increasing order'
                );
        ...
        describe("Token to Token (T2T)", function() {
            before('one time init of vars', async() => {
                t2eReserves = ['0xff1234567a334f7d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xaa7777cc1234500f'];
                e2tReserves = ['0xff12345663820d8f', '0xaa12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057'];
                t2eDupReserves = ['0xaa12aa56bbfff000', '0xcc12345675fff057', '0xcc12345675fff057', '0xff1234567a334f7d'];
                e2tDupReserves = ['0xaa12345616709a5d', '0xaa12345616709a5d', '0xaa12aa56bbfff000', '0xff1234567a334f7d'];
                t2eMissingReserves = ['0xff1234567a334f7d', '0xbb12aa56bbfff000', '0xcc12345675fff057', '0xaa7777cc1234500f']; // 0xbb12aa56bbfff000 doesn't exist
                e2tMissingReserves = ['0xff12345663820d8f', '0xbb12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057']; // 0xbb12345616709a5d doesn't exist
                t2eAscendingReserves = ['0xaa12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xff1234567a334f7d'];
                e2tAscendingReserves = ['0xaa12345616709a5d', '0xaa12aa56bbfff000', '0xcc12345675fff057', '0xff12345663820d8f'];
                t2eDescendingReserves = ['0xff1234567a334f7d', '0xcc12345675fff057', '0xaa7777cc1234500f', '0xaa12aa56bbfff000'];
                e2tDescendingReserves = ['0xff12345663820d8f', '0xcc12345675fff057', '0xaa12aa56bbfff000', '0xaa12345616709a5d'];
            });
